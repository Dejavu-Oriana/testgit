...
public class AlbumDetailViewModel : ViewModelBase
{
    ...
    private string _originalSelectedImagePath = string.Empty; // 存储原始选中的图片路径
    private Bitmap? _coverImage; // 封面图片，用于预览
    ...
    /// <summary>
    /// 封面图片（用于预览，与AddAlbumViewModel保持一致）
    /// </summary>
        ...
        try
        {
            ...
            if (Uri.TryCreate(coverUrl, UriKind.Absolute, out var coverUri))
            {
                ...
                if (coverUri.Scheme == "file")
                {
                    ...
                    {
                        CoverImage = new Bitmap(localPath);
    ...
    private void Reset()
    {
        ...
        // 重置封面相关属性
        _originalSelectedImagePath = null; // 清除原始选中的图片路径
        LoadCoverImage(_selectedAlbum); // 重新加载原始封面
    ...
    private async void PickCover()
    {
        try
        {
            ...
            if (files != null && files.Length > 0 && SelectedAlbum != null)
            {
                ...
                // 存储原始选中的图片路径，仅在保存专辑时使用
                _originalSelectedImagePath = selectedFile;
                
                // 直接从原始文件路径加载CoverImage用于预览，与AddAlbumViewModel保持一致
                try
                {
                    CoverImage = new Bitmap(selectedFile);
                    System.Console.WriteLine("CoverImage loaded successfully from original path");
                    ErrorMessage = string.Empty;
                }
                catch (Exception ex)
                {
                    ErrorMessage = $"加载封面图片失败: {ex.Message}";
                    System.Console.WriteLine("Failed to load CoverImage: " + ex.Message);
                    CoverImage = null;
