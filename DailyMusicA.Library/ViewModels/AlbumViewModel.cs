...
public class AlbumViewModel : ViewModelBase
{
    ...
    /// 刷新专辑命令
    /// </summary>
    public virtual ICommand RefreshAlbumsCommand { get; }
        ...
        {            // 初始化数据库连接
            ...
            foreach (var album in albums)
            {
                ...
                if (!string.IsNullOrEmpty(album.CoverUrl))
                {
                    ...
                    try
                    {
                        if (Uri.TryCreate(album.CoverUrl, UriKind.Absolute, out var coverUri))
                        {
                            // 如果是file://协议，直接使用本地路径
                            if (coverUri.Scheme == "file")
                            {
                                ...
                                if (System.IO.File.Exists(localPath))
                                {
                                    ...
                                    System.Console.WriteLine($"AlbumViewModel: 成功加载专辑封面图片: {localPath}");
                                }
                            }
                        }
                    }
                    ...
                    {
                        System.Console.WriteLine($"AlbumViewModel: 加载专辑封面失败: {ex.Message}");
                    }
                }
                
                System.Console.WriteLine($"AlbumViewModel: 处理专辑 - ID: {album.Id}, 名称: {album.Name}, 艺术家: {album.Artist}");
                System.Console.WriteLine($"AlbumViewModel: 专辑封面URL: {album.CoverUrl}");
