    ...
    public async Task DeleteAlbumAsync(string id)
    {
        try
        {
            ...
            if (album != null && !string.IsNullOrEmpty(album.CoverUrl))
            {
                    ...
                    if (Uri.TryCreate(album.CoverUrl, UriKind.Absolute, out var coverUri))
                    {
                        ...
                        if (coverUri.Scheme == "file")
                        {
                            ...
                            if (System.IO.File.Exists(localPath))
                            {
                                ...
                                System.Console.WriteLine($"删除专辑封面照片成功: {localPath}");
                            }
                        }
                    }
                }
                ...
                {
                    // 记录删除照片失败的错误，但不影响后续删除数据库记录的操作
                    System.Console.WriteLine($"删除专辑封面照片失败: {ex.Message}");
                }
            }
        }
        catch (Exception ex)
        {
            // 记录获取专辑信息失败的错误，但不影响后续删除数据库记录的操作
            System.Console.WriteLine($"获取专辑信息失败: {ex.Message}");
        }
